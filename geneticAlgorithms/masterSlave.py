...
import abc
from . import masterSlaveFitness
...
class MasterSlave(geneticBase.GeneticAlgorithmBase, metaclass=abc.ABCMeta):
    ...
    def _send_individuals_reproduce(self):
        ...
        results = list(futures.map(masterSlaveFitness.fitness, self._population))
        neighbours = Collect()
        ...
        fit_values = list(futures.map(masterSlaveFitness.fitness, self._population))
    ...
    def _find_solution(self, population):
        ...
        """
        results = list(futures.map(masterSlaveFitness.fitness, population))
        neighbours = Collect()
        while neighbours.size_of_col() != self._population_size:
            fit_val, chromosome = results.pop(0)
            neighbours.append_object(Snt(fit_val, chromosome))
